...
int tcp_receive (tcpsock_t * socket, void * buffer, int * buf_size, int error)
{
  if(error != TCP_NO_ERROR)return error; //do nothing because the previous action ended allready with an error but the error wasn't catched yet
  TCP_ERR_HANDLER(socket==NULL,return TCP_SOCKET_ERROR);
  TCP_ERR_HANDLER(socket->cookie!=MAGIC_COOKIE,return TCP_SOCKET_ERROR); 
...
int tcp_set_sd(tcpsock_t * socket, int * sd)
{
  TCP_ERR_HANDLER(socket==NULL,return TCP_SOCKET_ERROR);
  TCP_ERR_HANDLER(socket->cookie!=MAGIC_COOKIE,return TCP_SOCKET_ERROR); 
  socket->sd = *sd;
